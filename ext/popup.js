...
function addAction(appsDiv, app, selected) {
  var div = document.createElement('div');
  div.className = 'action' + (selected ? ' action_selected' : '');

  div.onclick = function() {
    launchSelection(selection.id);
  };

  var img = document.createElement('img');
  img.src = getIconURL(action);
  div.appendChild(img);

  var title = document.createElement('span');
  title.className = 'action_title';
...
function reloadActionDisplay() {
  ...
  for (var i = 0; i < actionList.length; i++) {
    ...
    addAction(actionsDiv, item, i == selectedIndex);
  }
}
...
// Puts only enabled actions from completeList into actionList.
function rebuildActionList(filter) {
  selectedIndex = 0;
  actionList = [];
  for (var i = 0; i < completeList.length; i++){
    var item = completeList[i];
    // Skip extensions and disabled actions.
    if (!item.isAction || !item.enabled) {
      continue;
    }
    if (filter && item.name.toLowerCase().search(filter) < 0) {
      continue;
